...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    client.auth.getUser().then(({ data, error }) => {
      ...
      acceptUser(data.user).catch((claimError) => {
        setAuthMessage(claimError instanceof Error ? claimError.message : "Account-Übernahme fehlgeschlagen.");
        setIsLoadingAuth(false);
      });
    });
  ...
  const signUp = async (email: string, password: string) => {
    ...
    if (!data.session) setAuthMessage("Registrierung erfolgreich. Bitte bestätige deine E-Mail und melde dich danach an.");
  };
  const signOut = async () => {
  ...
  return { user, isLoadingAuth, authMessage, setAuthMessage, signIn, signUp, signOut };
}
