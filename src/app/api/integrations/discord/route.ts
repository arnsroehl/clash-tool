...
export async function POST(request: NextRequest) {
  ...
  let webhook: URL;
  try {
    webhook = new URL(body.webhookUrl || "");
  } catch {
    return NextResponse.json(
      { error: "Ungültige Discord-Webhook-URL." },
      { status: 400 },
    );
  }
  const allowedHost =
  ...
  if (
    ...
    return NextResponse.json(
      { error: "Erlaubt sind ausschließlich offizielle Discord-Webhook-URLs." },
      { status: 400 },
    );
  ...
  if (!summary)
    return NextResponse.json(
      { error: "Die Planung ist leer." },
      { status: 400 },
    );
  const discordResponse = await fetch(webhook, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ content: `**Clash-Tool-Planung**\n${summary}` }),
    cache: "no-store",
    redirect: "error",
  });
  if (!discordResponse.ok)
    return NextResponse.json(
      { error: "Discord hat den Webhook abgelehnt." },
