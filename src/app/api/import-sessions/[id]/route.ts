...
export async function GET(request: NextRequest, context: Context) {
  ...
  const [session, files, changes, jobs] = await Promise.all([
    ...
    auth.client.from("screenshot_import_files").select("id, original_filename, width, height, screen_type, screen_type_confidence, quality_score, quality_issues, processing_status, created_at, deleted_at").eq("import_session_id", id).order("created_at"),
