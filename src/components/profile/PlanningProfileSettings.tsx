...
export function PlanningProfileSettings({ profile, onChange }: Props) {
  return <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <label className="text-sm text-slate-300">Sprache<select value={profile.language} onChange={(event) => onChange({ ...profile, language: event.target.value as PlanningProfile["language"] })} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950 p-3"><option value="de">Deutsch</option><option value="en">English</option></select></label>
