...
export async function getPlanningProfile(
  ...
  const { data, error } = await client
    ...
    .select(
      "user_id,play_style,language,reminders_enabled,daily_summary_enabled",
    )
    .eq("user_id", userId)
    .maybeSingle();
...
export async function savePlanningProfile(
  ...
  const { error } = await getSupabaseClient()
    ...
    .upsert({
      ...
      daily_summary_enabled: profile.dailySummaryEnabled,
      updated_at: new Date().toISOString(),
    });
  if (error) throw new Error(error.message);
}

export async function getPlanningGoals(
  accountId: string,
): Promise<PlanningGoal[]> {
  const { data, error } = await getSupabaseClient()
    .from("planning_goals")
    .select(
      "id,account_id,item_type,item_id,name,current_level,target_level,target_date,estimated_hours,status",
    )
...
export async function createPlanningGoal(
  input: Omit<PlanningGoal, "id" | "status">,
): Promise<PlanningGoal> {
  const { data, error } = await getSupabaseClient()
    .from("planning_goals")
    .insert({
...
export async function updatePlanningGoalStatus(
  id: string,
