...
export async function enableWebPush(): Promise<void> {
  ...
  if (!publicKey) throw new Error("Web Push ist auf dem Server noch nicht vollständig konfiguriert.");
  ...
  if (!json.endpoint || !json.keys?.p256dh || !json.keys.auth) throw new Error("Der Browser hat kein vollständiges Push-Abonnement erstellt.");
  const client = getSupabaseClient();
  const { data: { user } } = await client.auth.getUser();
  if (!user) throw new Error("Bitte melde dich erneut an.");
  const { error } = await client.from("push_subscriptions").upsert({ user_id: user.id, endpoint: json.endpoint, p256dh: json.keys.p256dh, auth_key: json.keys.auth, user_agent: navigator.userAgent, updated_at: new Date().toISOString() }, { onConflict: "endpoint" });
...
export async function sendTestPush(): Promise<void> {
  ...
  if (!response.ok) throw new Error(data.error || "Test-Push konnte nicht gesendet werden.");
}
